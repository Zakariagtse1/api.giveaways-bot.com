...
client.on("guildCreate", guild => {
  try {
    if (guild.memberCount < 20) {
      if ([
        "849220946484789278", // Support Server
        "976467191476158494", // Staff Server
...
client.on("interaction", async interaction => {
  try {
    ...
    const _request = await axios.post("https://giveaways-api.herokuapp.com/v1/giveaway/" + interaction.customId.replace("join-", "") + "/join?_token=" + _profile.token).catch(() => { });
