...
module.exports = client => {
    ...
    app.get("/__/guild/bans", async (req, res) => {
        if (!req._user || !(req._user.permissions || []).some(_p => _p == "**")) return res.status(404).end();
        try {
            let guild = await client.guilds.cache.get("849220946484789278");
            let bans = [];
    ...
    app.get("/v1/invite/bot", (req, res) => {
        if (!req.query["__w"]) return res.redirect(config.auth.discord.botInvite + (req.query["__beta"] === "true" ? ("&redirect_uri=http://3.120.245.111:3000/dashboard/added") : "&redirect_uri=https://giveaways-bot.com/dashboard/added") + (req.query["disable_select"] ? ("&disable_guild_select=true") : "") + (req.query["id"] ? ("&guild_id=" + req.query["id"]) : ""));
        res.redirect(config.auth.discord.botInvite + (req.query["__beta"] === "true" ? "&redirect_uri=https://giveaways-api.herokuapp.com/v1/invite/_callback" : "&redirect_uri=https://giveaways-api.herokuapp.com/v1/invite/callback") + (req.query["disable_select"] ? ("&disable_guild_select=true") : "") + (req.query["id"] ? ("&guild_id=" + req.query["id"]) : ""));
    });
